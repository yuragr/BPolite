...
public class EventInstanceService extends IntentService {
			...
			if (type != null && !type.isEmpty() && eventInstanceHashCode != 0 && calendarHashCode != 0
				...
				if (eventInstanceExists(eventInstanceHashCode, calendar)) {
					...
					if (eventType.equals(EventInstanceType.START)) {
						silenceRinger(audioManager, ringerMode, startTime, endTime, delayTime);
					}
				...
				Log.w(this.getClass().getSimpleName(), "nothing was done");
			}
		}
		Log.d(this.getClass().getSimpleName(), "<< onHandleIntent");
	}
	...
	private boolean eventInstanceExists(int eventInstanceHashCode, Calendar calendar) {
		...
		HashSet<EventInstance> eventInstances = EventInstanceRepository.getEventInstancesFromXml(this);
	...
	private void restoreRinger(AudioManager audioManager, int ringerMode, long startTime, long endTime,
		...
		synchronized (syncLock) {
			if (ringerMode != AudioManager.RINGER_MODE_NORMAL) {
				 ...
				 */
				HashSet<EventInstance> eventInstances = EventInstanceRepository
	 ...
	 * @return
	 */
	private boolean allowedToSilenceOnWeekDay() {
		boolean result = false;
